...
本專案已實作基本的資安防護機制，請務必遵守以下 API 認證機制：

1.  **隱私保護**：Wi-Fi 與 API 金鑰存放在 `esp32/secrets.h`，此檔案已被加入 `.gitignore`，請勿將其上傳至公開倉庫。
2.  **API 認證**：所有對 ESP32-CAM 的 HTTP 請求必須攜帶 `auth` 參數。
    *   範例：`http://{IP}/stream?auth={YOUR_API_KEY}`
3.  **金鑰同步**：若要更改金鑰，請同時修改 `esp32/secrets.h` 與 `pi/config.json` 中的 `api_key` 欄位。
4.  **CORS 安全**：管理介面後端已限制僅允許特定來源訪問。若需遠端管理，建議搭配 VPN 或 SSH Tunnel。

---

## 🌐 網路排錯指南 (Network Troubleshooting)

若您在管理介面看到 "TCP 握手失敗" 或影像無法顯示，請檢查以下幾點：

1. **檢查網段是否一致**：
   - 確保 Raspberry Pi 與 ESP32-CAM 連接在同一個 Wi-Fi 路由器下。
   - 進入「系統設定」頁面，檢查上方顯示的「區域網路 IP」是否與攝影機 IP 在同一個網段 (例如都是 `192.168.50.x`)。
2. **使用連線測試工具**：
   - 在「系統設定」頁中，使用「連線測試 (TCP)」功能，輸入攝影機 IP 進行測試。
   - 若顯示「認證失敗」但有通訊，請檢查 ESP32 端的 `secrets.h` 中的 API Key 是否與 `config.json` 一致。
3. **mDNS 自動發現**：
   - 系統會自動尋找區域網路中的攝影機，這依賴於 mDNS 服務。請確保您的路由器沒有阻擋 mDNS 封包（名稱通常為 `esp32-safety`）。
4. **硬體隔離 (AP Isolation)**：
   - 某些公共 Wi-Fi 或路由器開啟了「無線隔離」功能，這會導致 Pi 無法連線到 ESP32，請務必將此功能關閉。
5. **模擬器測試**：
   - 若實體連線受阻，可先執行 `python pi/mock_esp32.py` 並在設定中新增 `127.0.0.1:8080` 作為攝影機進行測試。
